...
public class VehicleControl : MonoBehaviour
{
    ...
    public Transform carSteer;//方向盘


    public Light[] brakeLights;//刹车灯.
    public AudioClip brakeSound;
    public GameObject brakeParticle;

    public AudioSource switchGear;//换挡声音.
    

    private GameObject[] Particle = new GameObject[4];

	public float suspensionDistance = 0.2f; // 在悬架运动量,1时,汽车弹起一个轮子的高度.
    public float springs = 1000.0f; // 悬载系统弹性.
    public float dampers = 2f; // 悬载系统的阻尼大小.相当于弹簧的硬度.
    public bool AutomaticWheelRadius = true;
    public float wheelRadius = 0.25f; // the radius of the wheels
    public float torque = 100f; // 引擎的动力.每个轮子的扭矩力.the base power of the engine (per wheel, and before gears)
    public float brakeTorque = 2000f; // 刹车的动力.the power of the braks (per wheel)
    public float wheelWeight = 3f; // the weight of a wheel
	/**
	 * 重心的位置.
	 */
    public Vector3 shiftCentre = new Vector3(0.0f, -0.25f, 0.0f); // 重心的位置.offset of centre of mass

    public float maxSteerAngle = 30.0f; // 轮子的最大转角.max angle of steering wheels

    public JWheelDrive wheelDrive = JWheelDrive.Front; // 驱动类型.默认前轮驱动.which wheels are powered

    public float shiftDownRPM = 1500.0f; // rpm就是换挡的马达转速.rpm script will shift gear down
    public float shiftUpRPM = 2500.0f; // rpm script will shift gear up
    public float idleRPM = 500.0f; // idle rpm

    public float stiffness = 0.1f; //轮子的滑动因子.值越大,加速度越大.刹车也越快. for wheels, determines slip

	public float brakeNess = 0.02f; //漂移滑动因子.值越大,偏移转弯越快.0.02慢,0.5快.
    // 齿轮转速比.gear ratios (index 0 is reverse)
    public float[] gears = { -10f, 9f, 6f, 4.5f, 3f, 2.5f };

    // 自动换挡automatic, if true car shifts automatically up/down
    public bool automatic = true;
	//在一定的转速效率/表，在tablestep转速增加，1.0f的效率为100%
	//在给定转速，电流表有100%在2000rpm
	...
	//在表的索引的规模，所以250F，750rpm为efficiencytable [ 3 ]。
    ...
    private float wantedRPM = 0.0f; //引擎简要达到的rpm rpm the engine tries to reach
    private float curRotation;
    private float brackF = 0.0f;



	//每一轮有一个wheeldata结构，包含有用的轮具体信息
	...
	private Vector3 useHuo=new Vector3();
    void FixedUpdate()
    {
		...
		bool congci = false;
            ...
            if ((checkForActive == null) || checkForActive.active)
            {
				...
				congci=Input.GetButton("Submit");
            }
		GameObject ga=GameObject.Find ("huo0");
		if (congci) {
			ga.particleSystem.Play ();
			if(useHuo.magnitude==0)
			{
				useHuo=rigidbody.velocity.normalized*1;
			}
			rigidbody.velocity+=useHuo;
		} else {
			if(useHuo.magnitude!=0)
			{
        ...
        foreach (WheelData w in wheels)
        {
            ...
            if (brake)
            {
				col.steerAngle = Mathf.Lerp(col.steerAngle, steer * w.maxSteer,brakeNess);// 0.02f);
            }
            else 
            {
