...
//				//从数据库或一个全局变量中得到 
//				this.LabelFenshuwenzi.GetComponent<UILabel> ().text = RaceManager.Instance.RaceCounter.bounceScore.ToString ();
...
//				this.LabelGuoguan.GetComponent<UILabel> ().text = "999";
				...
				if(RaceManager.Instance.RaceCounterInstance!=null)
					bounceScore = RaceManager.Instance.RaceCounterInstance.bounceScore;
				else
					bounceScore = 0;

				DOVirtual.Float(0,bounceScore,2,delegate(float value) {
					this.LabelFenshuwenzi.GetComponent<UILabel> ().text = ((int)value).ToString();
				});
