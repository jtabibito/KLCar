...
/// 2015-4-14 13:08:16
/// </summary>
public partial class ContainerOperationjixiansaiUIController : UIControllerBase
{
	/// 宠物CD恢复进度相关变量
	...
	private float 	   mFloatDashBoardMaxSpeed = 500.0f;
	...
	///
	private UISprite mUISpriteFuelCapProgress;							//油量图片UISprite
	private float	 mFloatFuelCapProgressStep = 0.05f;					//后续从Excel表格中进行初始化
	...
	void Start ()
	{
		...
		this.mUISpriteFuelCapProgress = this.SpriteJiayout.GetComponent<UISprite> ();
		if (this.mUISpriteFuelCapProgress != null) {
			this.mUISpriteFuelCapProgress.type = UISprite.Type.Filled;
			this.mUISpriteFuelCapProgress.fillDirection = UISprite.FillDirection.Horizontal;
			this.mUISpriteFuelCapProgress.flip = UISprite.Flip.Nothing;
			this.mUISpriteFuelCapProgress.invert = true;
			this.mUISpriteFuelCapProgress.fillAmount = 1.0f;
		}
...
//				this.ButtonZhanTingWenzhi.GetComponent<UIButton> ().onClick.Add (new EventDelegate (this.OnClickButtonZhanTingWenzhi));
	...
	void OnClickButtonFeidan ()
	{
		...
		if (this.mIntPropsFlyBomb <= 0) {
			...
			//					if (this.mIntPropsFlyBomb <= 0) {
			//						this.ButtonFeidan.GetComponent<UIButton> ().SetState (UIButton.State.Disabled, true);
			//					} else {
			//						this.ButtonFeidan.GetComponent<UIButton> ().SetState (UIButton.State.Normal, true);
			//						if (this.mBoolIsFlyBombEnableAgain == true) {
			//							this.ButtonFeidan.GetComponent<UIButton> ().SetState (UIButton.State.Normal, true);
			//							this.mBoolIsFlyBombEnableAgain = false;
			//						}
			//					}
	...
	void UpdateUIDisplay ()
	{
		...
		if (this.mUISpriteFuelCapProgress.fillAmount > 0.0f + Mathf.Epsilon) {
			//UpdateFuelCapProgress (Time.deltaTime * this.mFloatFuelCapProgressStep);
			UpdateFuelCapProgress ();
		} else {
			//游戏结束代码放到这里
		}
	...
	///////////////// 油量信息更新///////////////
	float UpdateFuelCapProgress ()
	{
		float restOilPercent = 0.0f;
		if(RaceManager.Instance.RaceCounterInstance!=null)
		{
			restOilPercent = RaceManager.Instance.RaceCounterInstance.oil/RaceManager.Instance.RaceCounterInstance.maxOil;
		}

		this.mUISpriteFuelCapProgress.fillAmount = restOilPercent;

//		this.mUISpriteFuelCapProgress.fillAmount = Mathf.Clamp01 (this.mUISpriteFuelCapProgress.fillAmount - Mathf.Clamp01 (percent));
//			
//		//是否和其他汽车发生了碰撞，发生了就减去大量的燃料
//		if (true) {
//			this.mUISpriteFuelCapProgress.fillAmount = Mathf.Clamp01 (this.mUISpriteFuelCapProgress.fillAmount - Mathf.Clamp01 (percent * 2));
//		}
//
//		//是否捡到了一个油量道具，捡到增加相应的燃油量
//		if (true) {
//			//暂时用随机数模拟
//			if (Random.Range (0, 10) == 4) {
//				Debug.Log ("开始加油");
//				this.mUISpriteFuelCapProgress.fillAmount = Mathf.Clamp01 (this.mUISpriteFuelCapProgress.fillAmount + Mathf.Clamp01 (0.2f * Time.deltaTime));
//			}
//		}

		return this.mUISpriteFuelCapProgress.fillAmount;
	}
		
	//捡到了道具
	void UpdateCollectFuelCapProgress (float percent)
	{
		//this.LabelJinbishuzi.GetComponent<UILabel> ().text = (tempValue++).ToString ();
	}
	...
	void UpdateDistance()
	{
		this.LabelXingshijuliJuqing.GetComponent<UILabel> ().text = RaceManager.Instance.RaceCounterInstance != null ? RaceManager.Instance.RaceCounterInstance.raceDistance.ToString () : "0";
	}
