...
public partial class ContainerOperationtiaozhansaiUIController : UIControllerBase
{
	...
	//按键的Enable只能运行一次，否则导致按键按下状态不会发生颜色变化
	private bool mBoolIsFlyBombEnableAgain = false;
	private bool mBoolIsShieldEnableAgain = false;
	private	bool mBoolIsHidingEnableAgain = false;
	private	bool mBoolIsSpeedEnableAgain = false;
	private GameObject petSkillObject = null;

	//或者用System.DataTime的方法得到时间信息
	private float mFloatAccumulateTime = 0.0f;
	private int mIntMinutes = 0;
	private int mIntSeconds = 0;
	private int mIntScroseconds = 0;
...
//				this.ButtonZhanTingWenzhi.GetComponent<UIButton> ().onClick.Add (new EventDelegate (this.OnClickButtonZhanTingWenzhi));
	...
	void Update ()
	{
		UpdateUIDisplay (Time.deltaTime);
	}
	...
	//////////////////////////////////////////////////////////////////////////
	void UpdateUIDisplay (float deltaTime)
	{
		if (IsPause == true)
			return;
		...
		UpdateTimeNumberLabel (deltaTime);
	...
	void UpdateTimeNumberLabel (float deltaTime)
	{
		this.mFloatAccumulateTime += deltaTime;
		
		this.mIntMinutes = ((int)this.mFloatAccumulateTime) / 60;
		this.mIntSeconds = ((int)this.mFloatAccumulateTime) - this.mIntMinutes * 60;
		this.mIntScroseconds = (int)(this.mFloatAccumulateTime * 100) - (int)(this.mFloatAccumulateTime * 100) / 100 * 100;
		
		string strMinutes = this.mIntMinutes < 10 ? ("0" + this.mIntMinutes) : (this.mIntMinutes.ToString ());
		string strSeconds = this.mIntSeconds < 10 ? ("0" + this.mIntSeconds) : (this.mIntSeconds.ToString ());
		string strMicroseconds = this.mIntScroseconds < 10 ? ("0" + this.mIntScroseconds) : (this.mIntScroseconds.ToString ());

		if (this.LabelDaojishifen != null)
			this.LabelDaojishifen.GetComponent<UILabel> ().text = strMinutes;
		if (this.LabelDaojishimiao != null)
			this.LabelDaojishimiao.GetComponent<UILabel> ().text = strSeconds;
		if (this.LabelDaojishihaomiao != null)
			this.LabelDaojishihaomiao.GetComponent<UILabel> ().text = strMicroseconds;
	}
	...
	{
		if (percent > 1.0f)
			Debug.LogError ("Pet cdTime is too large ,Please check");
